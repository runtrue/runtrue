-- Check updates are separate durable revisions of one provider check run.
-- Stable task/publication identities provide idempotency; these projections
-- must therefore permit multiple request digests for the same logical check.
ALTER TABLE scm_check_publications RENAME TO scm_check_publications_v27;
...
SELECT
    ...
    annotation_count, provider_check_run_id, confirmed_annotations, state,
    attempts, last_error_code, created_unix_ms, updated_unix_ms
FROM scm_check_publications_v27;

DROP TABLE scm_check_publications_v27;
