...
pub struct WasmExecutor {
    ...
    compiled: Mutex<BTreeMap<ContentDigest, Arc<AdmittedComponent>>>,
...
impl WasmExecutor {
    ...
    pub fn execute_request_with_adapters(
        ...
        let (component, cache_status) = self.admit_and_compile(reference)?;
        if request.cancellation.is_cancelled() {
            return Ok(WasmExecutionOutput {
        ...
        let grants = self.build_grants(request)?;
        ...
        let call_result = Run::instantiate(&mut store, &component.component, &linker)
            .and_then(|bindings| bindings.call_run(&mut store));
    ...
    fn admit_and_compile(
        &self,
        reference: &str,
    ) -> Result<(Arc<AdmittedComponent>, AotCacheStatus), WasmError> {
        let artifact = self
            .components
            .get(reference)
        ...
        if let Some(component) = self
            .compiled
            .lock()
            .map_err(|_| WasmError::Internal("compiled component state is poisoned".to_owned()))?
            .get(&digest)
            .cloned()
        {
            return Ok((component, AotCacheStatus::Hit));
        }
        let key = self.aot_cache_key(digest.clone());
        let inspection = match self.cache.inspect(&self.source_engine, &key) {
        ...
        let (component, status) = match inspection {
            Some(inspection) if inspection.status == AotCacheStatus::Hit => {
                let authenticated = inspection.authenticated_artifact.unwrap_or_default();
                match self.compile_component(&self.cached_engine, artifact, Some(&authenticated)) {
                    Ok((component, _)) => (component, AotCacheStatus::Hit),
                    Err(_) => {
                        let error = AotCacheError::CorruptArtifact;
                        self.record_cache_event(&key, &error);
                        let _ = self.cache.quarantine(&key);
                        let (component, _) =
                            self.compile_component(&self.source_engine, artifact, None)?;
                        (component, AotCacheStatus::QuarantinedMiss)
                    }
                }
            }
            Some(_) => {
                let (component, serialized) =
                    self.compile_component(&self.source_engine, artifact, None)?;
                self.record_compiled_aot(&key, &serialized);
                (component, AotCacheStatus::Miss)
            }
            None => {
                let (component, serialized) =
                    self.compile_component(&self.source_engine, artifact, None)?;
        ...
        if let Err(error) = self.cache.record(key, serialized) {
            let concurrent_exact = matches!(error, AotCacheError::EntryAlreadyExists)
                && self
                    ...
                    .is_some_and(|authenticated| authenticated == serialized);
            if !concurrent_exact {
                self.record_cache_event(key, &error);
                let _ = self.cache.quarantine(key);
            }
    ...
    fn compile_component(
        ...
        expected_authenticated: Option<&[u8]>,
    ) -> Result<(Arc<AdmittedComponent>, Vec<u8>), WasmError> {
